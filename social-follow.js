// Site-wide "Follow us" bar — X / Instagram / LinkedIn / TikTok. Auto-appends
// one instance at the very bottom of every page that loads this script, and
// fills in any in-page placeholder (<div data-follow-us></div>) with a
// matching bar — used by about.html to also show one near the top of the page.
(function () {
  const LINKS = [
    {
      name: 'Twitter',
      href: 'https://x.com/MMA_BRIDGE',
      svg: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>'
    },
    {
      name: 'Instagram',
      href: 'https://www.instagram.com/mma_bridge/',
      svg: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>'
    },
    {
      name: 'LinkedIn',
      href: 'https://www.linkedin.com/company/mma-bridge/',
      svg: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>'
    },
    {
      name: 'TikTok',
      href: 'https://www.tiktok.com/@mma_bridge',
      svg: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M16.6 5.82c-1.007-.982-1.594-2.335-1.594-3.82h-3.05v14.36c0 1.647-1.334 2.982-2.98 2.982a2.982 2.982 0 0 1-2.982-2.982 2.982 2.982 0 0 1 2.982-2.98c.24 0 .47.03.69.087v-3.12a6.13 6.13 0 0 0-.69-.04A6.03 6.03 0 0 0 3 16.36a6.03 6.03 0 0 0 6.03 6.03 6.03 6.03 0 0 0 6.03-6.03v-7.3a7.9 7.9 0 0 0 4.61 1.48V7.5a4.85 4.85 0 0 1-3.07-1.68z"/></svg>'
    },
  ];

  function iconsHTML() {
    return LINKS.map(l =>
      `<a class="sf-icon" href="${l.href}" target="_blank" rel="noopener noreferrer" aria-label="MMA Bridge on ${l.name}">${l.svg}</a>`
    ).join('');
  }

  function barHTML() {
    return `<span class="sf-label">Follow us</span><div class="sf-icons">${iconsHTML()}</div>`;
  }

  if (!document.getElementById('sf-styles')) {
    const style = document.createElement('style');
    style.id = 'sf-styles';
    style.textContent = `
      .sf-bar { display:flex; align-items:center; justify-content:center; gap:20px; flex-wrap:wrap; padding:24px 20px; }
      .sf-bar.sf-footer { border-top:1px solid rgba(255,255,255,.07); margin-top:32px; }
      .sf-label { font-family:'Montserrat',sans-serif; font-weight:700; font-size:.82rem; color:rgba(255,255,255,.5); letter-spacing:.01em; }
      .sf-icons { display:flex; align-items:center; gap:12px; }
      .sf-icon {
        display:flex; align-items:center; justify-content:center;
        width:36px; height:36px; border-radius:9px;
        background:rgba(255,255,255,.06); color:#fff;
        transition:background .18s, transform .18s, color .18s;
      }
      .sf-icon:hover { background:linear-gradient(165deg, #ffb066 0%, #ff8a3d 45%, #e8691c 100%); color:#0a0a0a; transform:translateY(-2px); }
      .sf-icon svg { width:17px; height:17px; display:block; }
      body.light-mode .sf-bar.sf-footer { border-top-color:rgba(0,0,0,.08); }
      body.light-mode .sf-label { color:rgba(0,0,0,.45); }
      body.light-mode .sf-icon { background:rgba(0,0,0,.05); color:#161514; }
      body.light-mode .sf-icon:hover { background:linear-gradient(165deg, #ffb066 0%, #ff8a3d 45%, #e8691c 100%); color:#0a0a0a; }
    `;
    document.head.appendChild(style);
  }

  // Fill any in-page placeholder (e.g. about.html's top-of-page instance).
  document.querySelectorAll('[data-follow-us]').forEach(el => {
    el.classList.add('sf-bar');
    el.innerHTML = barHTML();
  });

  // Always append one footer instance at the very bottom of the page.
  const footer = document.createElement('div');
  footer.className = 'sf-bar sf-footer';
  footer.innerHTML = barHTML();
  document.body.appendChild(footer);
})();

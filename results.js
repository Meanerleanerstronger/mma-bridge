(function () {
  ...
  function fighterCard(f) {
    ...
    const badge   = isChamp ? `<span style="background:rgba(200,150,10,.14);border:1px solid rgba(200,150,10,.4);color:#c9a227;font-size:.58rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;border-radius:20px;padding:2px 8px;">Champ</span>` :
                    isPfp   ? `<span style="background:rgba(240,180,41,.08);border:1px solid rgba(240,180,41,.22);color:#f0b429;font-size:.58rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;border-radius:20px;padding:2px 8px;">★ P4P</span>` : '';
    return `
  ...
  function eventCard(ev) {
    ...
    const statusTag = isPast(ev)
      ...
      : `<span style="color:#f0b429;">Upcoming</span>`;
    return `
    ...
    if (!total) {
      cardsEl.innerHTML = `
        <div class="results-empty" style="grid-column:1/-1">
          ...
          <div class="results-empty-sub">Try a fighter nickname, weight class, or country. Or <a href="lucas.html" style="color:#f0b429;">ask Lucas</a>.</div>
        </div>`;
      return;
    }

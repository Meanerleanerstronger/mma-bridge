...
function renderHeroSplit(completedEv, upcomingEv) {
  ...
  section.innerHTML = `
    ...
    <!-- DIVIDER -->
    <div style="position:absolute;left:50%;top:8%;bottom:8%;width:1px;background:linear-gradient(to bottom,transparent 0%,rgba(242, 96, 15,0.45) 40%,rgba(242, 96, 15,0.45) 60%,transparent 100%);transform:translateX(-50%);z-index:10;pointer-events:none;"></div>
...
function checkEventNotifications(events) {
  try {
    ...
    banner.style.cssText = 'position:fixed;top:70px;left:50%;transform:translateX(-50%);z-index:9998;background:linear-gradient(135deg,#0a1820,#0d2538);border:1px solid rgba(242, 96, 15,0.28);border-radius:12px;padding:14px 18px;max-width:380px;width:calc(100% - 40px);box-shadow:0 8px 40px rgba(0,0,0,0.6);display:flex;align-items:center;gap:12px;animation:sectionFadeIn 0.4s ease both;';
    banner.innerHTML = `
      <span style="flex-shrink:0;display:flex;align-items:center;"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(242, 96, 15,0.8)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg></span>
      ...
      <a href="events.html?id=${encodeURIComponent(target.id)}" style="flex-shrink:0;background:rgba(242, 96, 15,0.09);border:1px solid rgba(242, 96, 15,0.28);color:cyan;font-family:Montserrat,sans-serif;font-size:0.65rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;padding:7px 12px;border-radius:6px;text-decoration:none;">View</a>
...
function renderRecentResults(events) {
  ...
  function buildCard(ev) {
    ...
    const defCol  = isPPV ? '#f0c34e' : '#f2600f';
    const hoverBorder = isPPV ? 'rgba(240,195,78,0.4)' : 'rgba(242, 96, 15,0.3)';
    ...
    card.onmouseenter = () => { card.style.borderColor=hoverBorder; card.style.transform='translateY(-3px)'; };
    ...
    card.innerHTML = `
      ...
      <div style="padding:14px 16px;">
        ...
        ${winner ? `
          <div style="font-family:'Inter',sans-serif;font-size:0.76rem;color:${defCol};font-weight:600;">${winner} def. ${loser}</div>
...
async function renderReddit() {
  ...
  try {
    ...
    const flairColors = {
      'Discussion': '#f2600f', 'News': '#c24a08', 'Video': '#ef4444',
      'Meme': '#a855f7', 'Highlight': '#22c55e', 'Ranking': '#c24a08',
    };
...
const REVIEW_BTN_PALETTE = [
  ...
  { bg: '#f2600f', border: '#c9601a', text: '#2b1200' }, // orange
